...
PROJECT_JSON=$(curl -fsS -X POST "$API/projects" \
  ...
  -H 'Content-Type: application/json' \
  -d "{\"name\":\"E2E P2 KB $(date +%s%N)\",\"description\":\"P2 knowledge test\",\"accessType\":\"MEMBERS_ONLY\"}")
...
REQ_FOLDER=$(curl -fsS -X POST "$API/projects/$PROJECT_ID/folders" \
  -H "Authorization: Bearer $ADMIN_TOKEN" -H 'Content-Type: application/json' \
  -d '{"name":"Requirements","description":"Requirements area"}')
REQ_ID=$(echo "$REQ_FOLDER" | python3 -c 'import json,sys; print(json.load(sys.stdin)["id"])')

TECH_FOLDER=$(curl -fsS -X POST "$API/projects/$PROJECT_ID/folders" \
  -H "Authorization: Bearer $ADMIN_TOKEN" -H 'Content-Type: application/json' \
  -d "{\"name\":\"Architecture\",\"description\":\"Architecture\",\"parentId\":\"$REQ_ID\"}")
ARCH_ID=$(echo "$TECH_FOLDER" | python3 -c 'import json,sys; print(json.load(sys.stdin)["id"])')

depth_status=$(curl -s -o /dev/null -w '%{http_code}' -X POST "$API/projects/$PROJECT_ID/folders" \
  -H "Authorization: Bearer $ADMIN_TOKEN" -H 'Content-Type: application/json' \
  -d "{\"name\":\"Too Deep\",\"description\":\"blocked\",\"parentId\":\"$ARCH_ID\"}")
if [[ "$depth_status" != "400" ]]; then
  echo "Expected 400 for third folder level, got $depth_status"
  exit 1
fi

LINK_JSON=$(curl -fsS -X POST "$API/projects/$PROJECT_ID/items/links" \
  -H "Authorization: Bearer $ADMIN_TOKEN" -H 'Content-Type: application/json' \
  -d "{\"folderId\":\"$ARCH_ID\",\"title\":\"API Documentation\",\"description\":\"Swagger\",\"category\":\"KT_DOCUMENTS\",\"externalUrl\":\"https://example.com/api\"}")
ITEM_ID=$(echo "$LINK_JSON" | python3 -c 'import json,sys; print(json.load(sys.stdin)["id"])')

curl -fsS "$API/projects/$PROJECT_ID/items?search=api&sourceType=LINK" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  | python3 -c 'import json,sys; assert json.load(sys.stdin)["totalElements"]>=1'

curl -fsS "$API/projects/$PROJECT_ID/items/$ITEM_ID/link" -H "Authorization: Bearer $ADMIN_TOKEN" \
  | python3 -c 'import json,sys; assert json.load(sys.stdin)["accessCount"]==1'
...
curl -fsS -X DELETE "$API/projects/$PROJECT_ID/folders/$ARCH_ID" -H "Authorization: Bearer $ADMIN_TOKEN" >/dev/null
